...
from ros2_ws.src.wayf_vision.wayf_vision.bbox_utils import create_bbox3d_array, depth2PointCloud, detections_to_pose_array, detections_to_rviz_marker, add_box
...
class WayfindingTrackingNode(Node):
    def __init__(self, namespace, rotate=False, readCameraInfo=True, base_id='base_link'):
        super().__init__('wayfinding_tracking_node')
        self.rotate = rotate
        ...
        self.timer = self.create_timer(0.05, self.lookup_latest_transform)
    ...
    def process_human_points(self, color_img, depth_img, humans, timestamp):
        """
        ...
        for box in humans:
            ...
            pcd_mask[int(box[1]):int(box[3]), int(box[0]):int(box[2])] = 1  # Create mask for the bounding box
            ...
            if pointcloud.is_empty():
                ...
                continue
            pointcloud = pointcloud.voxel_down_sample(voxel_size=0.01)  # Downsample for efficiency
            if pointcloud.is_empty():
                self.get_logger().warn("Point cloud is empty after downsampling, skipping this detection")
                continue
            labels = np.array(pointcloud.cluster_dbscan(eps=0.02, min_points=10, print_progress=False))
            bins = np.bincount(labels[labels >= 0])
            if len(bins) == 0:
                self.get_logger().warn("No clusters found in point cloud, skipping this detection")
                continue
            largest_cluster = np.argmax(bins)
            pointcloud = pointcloud.select_by_index(np.where(labels == largest_cluster)[0])
...
def main(namespace, rotate, base_id, args=None):
    ...
    node = WayfindingTrackingNode(namespace=namespace, rotate=rotate, base_id=base_id)
